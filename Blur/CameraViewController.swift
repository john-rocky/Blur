...
class CameraViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate,AVCapturePhotoCaptureDelegate, AVCaptureMetadataOutputObjectsDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    lazy var coreMLRequest:VNCoreMLRequest = {
        let mode = try! VNCoreMLModel(for: semantic().model)
        let request = VNCoreMLRequest(model: mode, completionHandler: self.coreMLCompletionHandler)
        request.imageCropAndScaleOption = .scaleFit
        return request
    }()
    
    lazy var stepMLRequest:VNCoreMLRequest = {
        let mode = try! VNCoreMLModel(for: lab2().model)
    ...
    func coreMLCompletionHandler(request:VNRequest?,error:Error?) {
        ...
        let multiArray = result.pixelBuffer
        let ciimage = CIImage(cvPixelBuffer: multiArray)
        let handler = VNImageRequestHandler(ciImage: ciimage,options: [:])
        DispatchQueue.global(qos: .userInitiated).async {
            try? handler.perform([self.stepMLRequest])
        }
    }
    ...
    func coreMLCompletionHandler0(request:VNRequest?,error:Error?) {
        ...
        let cgImage = multiArray?.cgImage(min: 0, max: 1)
        var translate = CGAffineTransform()
        var ciImage = CIImage()
        var resize = CIImage()
        if  isVertical{
            resize = CIImage(cgImage: cgImage!).resizeToSameHeight(as: originalCIImage!)
            translate = CGAffineTransform(translationX: -(resize.extent.width - (originalCIImage?.extent.width)!) * 0.5, y: 0)
        } else {
            resize = CIImage(cgImage: cgImage!).resizeToSameWidth(as: originalCIImage!)
...
//            kCIInputBackgroundImageKey:translatedBack,
//            kCIInputMaskImageKey:translatedBack])
//        let out = filter?.outputImage
//            PortraitMatteImage = out!.cropped(to: originalCIImage!.extent)
        DispatchQueue.main.async {
